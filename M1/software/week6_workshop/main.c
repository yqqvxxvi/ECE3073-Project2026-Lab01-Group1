...
volatile int *OutPort_LEDR      = (int *) 0x04009070;  // LEDs
volatile int *OutPort_LED8      = (int *) 0x04009030;  // LED8
volatile int *InPort_KEY1       = (int *) 0x04009040;  // KEY1
volatile int *InPort_COUNTER    = (int *) 0x04009050;
volatile int *Counter_EdgeCap   = (int *) 0x0400905C;
volatile int *Counter_IRQMask   = (int *) 0x04009058;
volatile int *GPIO2_Port        = (int *) 0x04009060;
volatile int *OutPort_HEX0      = (int *) 0x04009020;
volatile int *UART              = (int *) 0x04009000;
volatile int *UART_control = (int *) 0x0400900C; // UART base + 0x0C
...
int main(void)
{
	...
	*UART_control = 0x80; // bit 7 = irrdy, enable interrupt on character received
	//3. enable Nios II interrupts (PIE bit in status register)
	NIOS2_WRITE_IENABLE(0x3); // 0x3 is binary 11, which sets both bit 0 (IRQ0) and bit 1 (IRQ1).
